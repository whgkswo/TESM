...
public class LinearSearcher {
    ...
    public SearchResult linearSearch(ServerWorld world, int maxReapetCount){
        ...
        for(int i = 0; i< maxReapetCount; i++){
            ...
            if(direction.isDiagonal()){
                SearchResult result;
                LinearSearcher xSearcher = new LinearSearcher(nextPos, endPos, new Direction(direction.getX(), 0));
                result = xSearcher.linearSearch(world, maxReapetCount-i-1);
                if(result.hasFoundDestination()){
                    return result;
                }else{
                    LinearSearcher zSearcher = new LinearSearcher(nextPos, endPos, new Direction(0, direction.getZ()));
